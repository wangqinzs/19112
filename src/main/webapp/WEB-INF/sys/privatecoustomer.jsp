...
<a class="layui-btn layui-btn-xs"lay-event="edit">编辑</a>
 <a class="layui-btn layui-btn-xs" lay-event="private"放弃客户</a>
</script>
...
<a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
 <a class="layui-btn layui-btn-xs" lay-event="private">放弃客户</a>
	...
	<table class="layui-table" lay-filter="test"
		lay-data="{url:'sys/privatetable',toolbar:'#toolbarDemo',id:'coustomerlist',page: true}">
		<thead>
			<tr>
				...
				<th lay-data="{field:'coustomer_man', width: '5%'}">联系人</th>
	...
	<script>
		...
		table.on('tool(test)', function(obj) {
			...
			if (obj.event === 'private') {
				layer.confirm('确定要放弃'+data.coustomer_name+'此客户吗', 
						function(index){
					/*发送请求，将该客户变成登录用户变成跟单客户  */
					$.post("sys/delprivate",data,function() {
						
						layer.msg('已放弃此客户', {icon: 6});
						table.reload("privatetable");
					})
				})
		...
		table.on('toolbar(test)', function(obj) {
			var cs = table.checkStatus("coustomerlist");
			switch (obj.event) {
			case 'select':
				table.reload("coustomerlist", {
					where : {
